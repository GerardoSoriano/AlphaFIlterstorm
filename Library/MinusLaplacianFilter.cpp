...
void MinusLaplacianFilter::bucle(uchar*& _input, uchar*& _output, uint _x, uint _y)
{
	int sum = 0;
...
void MinusLaplacianFilter::make_mask()
{
	...
	mask[1][0] = -weight;	mask[1][1] = (weight * 4) + 1;	mask[1][2] = -weight;
	mask[2][0] = 0;			mask[2][1] = -weight;			mask[2][2] = 0;
}

MinusLaplacianFilter::MinusLaplacianFilter(): Filter(), weight(1)
{
	make_mask();
}


MinusLaplacianFilter::~MinusLaplacianFilter()
{
}
...
void MinusLaplacianFilter::modify(int _weight)
{
	delete result;
	weight = _weight;
	make_mask();
